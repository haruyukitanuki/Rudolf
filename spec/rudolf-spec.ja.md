...
- `sentAt: string`：プロデューサー側におけるISO 8601形式のタイムスタンプ。タイムゾーン指定子を定義する必要があります。
...
日時: ISO 8601 形式で定義されます。実装はユースケースに依存します。
- 一般的な形式仕様
  - 西暦 0000 年から 9999 年まで（両端を含む）のみが許可されます。
- ヘッダー `sentAt`
  - タイムゾーン指定子を含める必要があります。これにより、異なるタイムゾーンで発行されたドキュメントの同期が可能になります。
