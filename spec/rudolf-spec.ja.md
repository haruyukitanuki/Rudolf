...
`sideOpened`は`int | null`です。規約は`stations.list[].doorSide`に準じますが、「どちら側か不明」を表す明示的な`null`が加わります。

- `-1` = 左側が開
- `0` = 閉（この車両のすべてのドアが閉：既知の閉状態）／なし／不明
- `1` = 右側が開
- `2` = 両側が開
- `null` = 開いているがside不明（シミュレーターが左右を区別できない）
...
- TCは車両ごとに1つのboolしか持ちません（左右の区別なし）：TCアダプターは、閉のとき`0`、開のとき`null`を出力します（左右をネイティブに判定できないため）。BVEアダプターは、実際の左右データ（`-1`／`1`／`2`）を出力します。
